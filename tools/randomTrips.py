...
def get_options(args=None):
    ...
    # Updated
    optParser.add_argument("--vehicle-class-prob", type=str,
                           default="100", help="probability for differerant vClasses")
    ...
    if options.vehicle_class:
        if options.tripprefix:
            ...
            options.vtypeID = options.vehicle_class.split("#")
...
def main(options):
    ...
    with open(options.tripfile, 'w') as fouttrips:
        ...
        if options.vehicle_class:
            vClassArray = options.vehicle_class.split('#');
            for idx in range(len(vClassArray)):
                vTypeDef = '    <vType id="%s" vClass="%s"%s/>\n' % (
                    options.vtypeID[idx], vClassArray[idx], vtypeattrs)
                if options.vtypeout:
                    ...
                    # "taxi", "taxi", vtypeattrs))


            # options.tripattrs += ' type="%s"' % options.vtypeID[1]
            personattrs += ' type="%s"' % options.vtypeID[0]
        depart = sumolib.miscutils.parseTime(options.begin)
        maxTime = sumolib.miscutils.parseTime(options.end)

        # Updated
        probability_distribution = options.vehicle_class_prob.split('#')
        for prob_idx in range(len(probability_distribution)):
            probability_distribution[prob_idx] = int(probability_distribution[prob_idx])/10;
        ...
        if trip_generator:
            if options.flows == 0:
                while depart < maxTime:
                    ...
                    vTypeIdx = random.choices(range(len(vClassArray)), probability_distribution)[0]
                    options.tripattrs = ' type="%s"' % options.vtypeID[vTypeIdx]
